    ...
    <>
        ...
        <div className="sticky top-6 space-y-5">
          {sortedBlocks.map((block, i) => {
            ...
            if (block.type === "quizCta") {
              return (
                <div key={i} className="rounded-xl border border-[#111111]/15 bg-[#111111]/[0.03] p-5">
                ...
                <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {selectedArticles.map((article) => (
                      ...
                      >
                        <p className="text-[13px] font-semibold leading-snug text-[#191919] group-hover:text-[#111111] transition-colors">
                          {article.title}
                        </p>
                        {article.category && (
      ...
      <div className="lg:hidden mt-10 space-y-6">
        {sortedBlocks.map((block, i) => {
          if (block.type === "quizCta") {
            return (
              <div key={i} className="rounded-xl border border-[#111111]/15 bg-[#111111]/[0.03] p-5 text-center">
                ...
                <Link
                  href={config.quizCta.ctaUrl}
                  className="mt-4 inline-flex h-[44px] items-center justify-center gap-1.5 rounded-lg bg-[#111111] px-6 text-[14px] font-bold text-white"
                >
                  {config.quizCta.ctaText}
                  <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.5} />
                </Link>
                  ...
                  {selectedProviders.map((provider, pi) => (
                    <div key={provider.id} className="flex items-center gap-3">
                      ...
                      <ProviderCta
                        ...
                        pageType={pageType}
                        sourceFlow={sourceFlow}
                        className="shrink-0 rounded-lg bg-[#111111] px-3 py-1.5 text-[12px] font-bold text-white"
                      >
                        Visit
                      </ProviderCta>
                    </div>
