...
// gated to ctx.vertical === "weight-loss"). This single-vertical ED build keeps
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-[1000px] px-4 py-8 sm:px-6">
          ...
          {review.trustBadges && review.trustBadges.length > 0 ? (
            ...
            <>
              <div className="flex items-center gap-2 text-[13px] text-gray-600">
                <Shield className="h-4 w-4 text-[#111111]" strokeWidth={1.5} />
                Licensed Providers
              </div>
              <div className="flex items-center gap-2 text-[13px] text-gray-600">
        ...
        {review.finalVerdict && (
          <div className="mb-8 rounded-2xl border border-[#111111]/20 bg-white p-5 shadow-sm sm:p-6">
            <p className="mb-2 text-[12px] font-bold uppercase tracking-[0.07em] text-[#111111]">The bottom line</p>
              ...
              {review.pricingPlans.map((plan) => (
                <div key={plan.name} className="rounded-xl border border-gray-200 bg-gray-50/60 p-5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {plan.cadence && (
                      <span className="rounded-full bg-[#111111]/10 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-[#111111]">
                        {plan.cadence}
                      </span>
                    )}
                  </div>
        ...
        {/* The Top TRT audit - verified-facts card. Registry-gated:
            ...
            <ol className="space-y-4">
              {review.howItWorks.map((step, i) => (
                <li key={i} className="flex gap-4">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#111111] text-[13px] font-bold text-white">
                    {i + 1}
                  </div>
                  <div>
                    {step.timing && (
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-[#111111]">
                        {step.timing}
                      </span>
                    )}
        ...
        {/* Mid-page CTA */}
        <div className="mb-6 rounded-xl border border-[#111111]/10 bg-[#111111]/[0.03] p-5 text-center sm:p-6">
          ...
          <ul className="space-y-2.5">
            {review.bestFor.map((item) => (
              <li key={item} className="flex items-start gap-2.5 text-[14px] text-gray-800">
                <ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-[#111111]" strokeWidth={2} />
                {item}
              </li>
            ))}
          </ul>
              ...
              {relatedBattles.map((battle) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-3 text-[14px] font-medium text-[#191919] transition-colors hover:border-[#111111]/30 hover:bg-[#111111]/[0.02]"
                  >
                    <span className="text-[#111111]">{provider.name} vs {otherProvider?.name}</span>
